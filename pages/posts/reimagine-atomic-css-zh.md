...
在正式开始前，先来聊聊背景。如果你还不认识我，我叫 jlhe，是 [Vite](https://vitejs.dev/) 团队的成员，也是 [Vitesse](https://github.com/jlhejs/vitesse) (Vite 社区最受欢迎的起手模板之一) 的作者。我享受原子化 CSS 带来的快速开发体验，而因此选择了 [Tailwind CSS](https://tailwindcss.com/) 作为 Vitesse 的默认 UI 框架。虽然 Vite 较 Webpack 等工具相比，在加载速度上有了大幅提升，但由于 Tailwind 生成了数 MB 的 CSS，使得加载与更新 CSS 成为了整个 Vite 应用的性能瓶颈。我曾以为这是使用为了原子式 CSS 的一种权衡，直到我发现了 [Windi CSS](https://cn.windicss.org)。
...
[**UnoCSS**](https://github.com/jlhejs/unocss) - 具有高性能且极具灵活性的即时原子化 CSS 引擎。
...
你可以参考 [文档](https://github.com/jlhejs/unocss#custom-variants) 了解更多细节。

### 预设

你可以将自己的自定义规则和可变修饰打包成预设，与他人分享，或是使用 UnoCSS 作为引擎创建你自己的原子化 CSS 框架！

同时，我们在发布时也提供了 [一些预设](https://github.com/jlhejs/unocss#presets) 供你快速上手。

值得一提的是，默认的 [`@unocss/preset-uno`](https://github.com/jlhejs/unocss/tree/main/packages/preset-uno) 预设（**实验阶段**）是一系列流行的原子化框架的 **通用超集**，包括了 Tailwind CSS，Windi CSS，Bootstrap，Tachyons 等。
...
[了解更多关于默认预设的信息](https://github.com/jlhejs/unocss/tree/main/packages/preset-uno)。
...
在 UnoCSS 中，我们也实现了属性化模式，只使用 [**一个可变修饰**](https://github.com/jlhejs/unocss/blob/main/packages/preset-attributify/src/variant.ts) 和 [**一个提取器**](https://github.com/jlhejs/unocss/blob/main/packages/preset-attributify/src/extractor.ts)，总共 **代码行数不超过 100**！更重要的是，它直接适用于你自定义的任何规则！
...
整个属性化模式是通过 [`@unocss/preset-attributify`](https://github.com/jlhejs/unocss/blob/main/packages/preset-attributify) 预设提供的，详细的使用方法请参考其文档。
...
同样的，这个功能的实现代码并未超过 100 行。具体请参考 [`@unocss/preset-icons`](https://github.com/jlhejs/unocss/blob/main/packages/preset-icons) 预设的实现了解其中的魔法。
...
考虑到 UnoCSS 带来的灵活性和想象力，坦率地说，我认为性能可能不是那么重要的事情。出于好奇，我写了一个 [简单的 benchmark](https://github.com/jlhejs/unocss/tree/main/bench) 来比较性能。结果令人惊讶：
...
顺便说一句，目前 [你正在阅读的网站](https://github.com/jlhejs/blog.qqxu.com) 就构建于 UnoCSS 之上，供你参考 :P。
...
非常感谢你的阅读！如果你对它感兴趣，请记得查看 [`antfu/unocss`](https://github.com/jlhejs/unocss) 仓库以了解更多细节，也可以通过 [**在线 Playground**](https://unocss.blog.qqxu.com/) 进行尝试。
